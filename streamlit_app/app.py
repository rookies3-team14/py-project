...
def load_stack_data_all_sheets():
    ...
    return {
        sheet: excel_file.parse(sheet).assign(sheet=sheet)
...
def load_github_interview_repos():
    path = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(path, "../data/excel/GithubInterviewRepos_Filtered.xlsx")
...
if selected:
        ...
        for row_categories in chart_rows:
            ...
            for i in range(4):
                if i < len(row_categories):
                    ...
                    cat_df = stack_df[
                        (stack_df["category"] == category) & (stack_df["count"] > 0)
                    ]
                    if not cat_df.empty:
                        with row_cols[i]:
    ...
    df["title"] = df.apply(
        lambda row: f'<a href="{row["recruitUrl"]}" target="_blank">{row["title"]}</a>',
        axis=1,
    )
    ...
    current_stack_df = github_df[
        github_df["stack"].str.strip().str.upper() == selected.upper()
    ].head(10)

    rows = [
        current_stack_df.iloc[i : i + 2] for i in range(0, len(current_stack_df), 2)
    ]
    for row in rows:
        cols = st.columns(2)
